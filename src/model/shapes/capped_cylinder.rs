...
impl Shape for CappedCylinder {
    ...
    fn as_capped_cylinder(&self) -> Option<&CappedCylinder> {
        Some(self)
    }
    fn as_capped_cylinder_mut(&mut self) -> Option<&mut CappedCylinder> {
        Some(self)
    }
    ...
    fn get_ui(&self, element: &Element, ui: &mut UI, _scene: &Arc<RwLock<Scene>>) -> UIElement {
        let mut category = UIElement::new("CappedCylinder", "cylinder", ElemType::Category(Category::default()), ui.uisettings());

        if let Some(cylinder) = element.shape().as_capped_cylinder() {
...
impl CappedCylinder {
    ...
    // Constructor
    pub fn new(pos: Vec3, dir: Vec3, radius: f64, height: f64) -> CappedCylinder {
        let dir = dir.normalize();
        let plane1 = Plane::new(pos.clone(), -dir.clone());
        let plane2 = Plane::new(pos.clone() + dir.clone() * height, dir.clone());
        let aabb = CappedCylinder::compute_aabb(pos.clone(), dir.clone(), height, radius);
        self::CappedCylinder { pos, dir, radius, height, plane: [plane1, plane2], aabb }
    }
    ...
    fn update_cylinder(&mut self) {
        ...
        let aabb = CappedCylinder::compute_aabb(self.pos.clone(), self.dir.clone(), self.height, self.radius);
        *self = self::CappedCylinder { pos: self.pos.clone(), dir: self.dir.clone(), radius: self.radius, height: self.height, plane: [plane1, plane2], aabb };
    }
