...
pub fn get_ray(scene: &Scene, x: usize, y: usize) -> Ray {
	...
	let dir = &top_left + &top_to_bot * (y as f64 / SCREEN_HEIGHT as f64) + &left_to_right * (x as f64 / SCREEN_WIDTH as f64) - scene.camera().pos();
	Ray::new(scene.camera().pos().clone(), dir.normalize(), 0)
}
